...
$(function() {
	...
	function multiFileDownload($filelist,$destination='DEFAULT/'){
		...
		$zipname=$destination."_".date('Ymd').".zip";
		$zip = new ZipArchive;
...
(function () {
	...
	function processUploader() {
		if (!empty($_FILES)) {
			...
			foreach ($oldtmpfiles as $file) {
				if(is_file($file))
					if($time - filemtime($file) >= 60*60*24) // 24 hour
						unlink($file);
			}
	...
	function processDropzone() {
		...
		if (!empty($_FILES)) {
			...
			$ok = move_uploaded_file($_FILES['file']['tmp_name'], $tmpfile);
			if (!$ok) $ok = rename($_FILES['file']['tmp_name'], $tmpfile);
			if ($ok) {
				$_FILES['file']['tmp_name'] = $tmpfile;
				echo json_encode($_FILES['file']);
			}
