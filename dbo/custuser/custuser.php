...
require(dirname(__FILE__).DIRECTORY_SEPARATOR.'custuser.conf.php');

# customization
function dbo_custuser_customize(&$dbo){
	global $DB, $USER;
...
function neworgrole(){
	...
	$toporgid = userTopOrgID();
	$orgdata = $DB->GetArray("select org_id,org_name from $tblorg where org_status = :0 and org_id = :1 and org_external = 'Y'", array('ACTIVE', $toporgid));
	$rs = $DB->getArray("select org_id,org_name from $tblorg where org_status = :0 and org_parentid = :1 and org_external = 'Y'", array('ACTIVE', $toporgid));
	if($rs)
...
function editorgrole($param1,$param12,$param3){
	...
	if(count($relationshiparr)>0){
		...
		foreach($relationshiparr as $key => $val){
			...
			$toporgid = userTopOrgID();
			// $orgdata = $DB->GetArray("select org_id,org_name from $tblorg where org_status = :0 and org_id = :1 and org_external = 'Y'",array('ACTIVE',$toporgid));
			$orgdata = $DB->GetArray("select org_id,org_name from $tblorg where org_status = :0 and org_external = 'Y'", array('ACTIVE'));
