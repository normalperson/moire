...
function dbo_artistsetup_custom_new($table, $cols){
	...
	foreach(array('password1', 'password2', 'userrole', 'skillids', 'orgids') as $tmp){
	...
	if($ok){
		# fcuserorgrole
		if($cols2['orgids']){
			$orgs = explode(', ', $cols2['orgids']);
			$roleid = $DB->getOne("select rol_id from fcrole where lower(rol_name) = 'artist'");
			$currentOrgs = $DB->getColumn("select uor_orgid from fcuserorgrole where uor_usrid = :0 and uor_rolid = :1", array($cols['usr_userid'], $roleid));
			$DB->execute("delete from fcuserorgrole where uor_usrid = :0 and uor_rolid = :1 and uor_orgid not in (".$cols2['orgids'].")", array($cols['usr_userid'], $roleid));
			foreach($orgs as $org){
				if(!in_array($org, $currentOrgs))
					$DB->execute("insert into fcuserorgrole (uor_usrid, uor_rolid, uor_orgid, uor_seq) values (:0, :1, :2, 1)", array($cols['usr_userid'], $roleid, $org));
			}
		}
		...
		# martistskill
		if($cols2['skillids']){
			$skills = explode(', ', $cols2['skillids']);
			$currentSkills = $DB->getColumn("select ma_jclid from martistskill where ma_usruserid = :0", array($cols['usr_userid']));
			$DB->execute("delete from martistskill where ma_usruserid = :0 and ma_jclid not in (".$cols2['skillids'].")", array($cols['usr_userid']));
			foreach($skills as $skill){
				if(!in_array($skill, $currentSkills))
					$DB->execute("insert into martistskill (ma_usruserid, ma_jclid) values (:0, :1)", array($cols['usr_userid'], $skill));
			}
		}
	}
	return $ret;
...
function dbo_artistsetup_custom_edit($table, $cols, $wheres){
	...
	if($ok){
		...
		if($cols2['skillids']){
			$skills = explode(', ', $cols2['skillids']);
			$currentSkills = $DB->getColumn("select ma_jclid from martistskill where ma_usruserid = :0", array($wheres['usr_userid']));
			$DB->execute("delete from martistskill where ma_usruserid = :0 and ma_jclid not in (".$cols2['skillids'].")", array($wheres['usr_userid']));
			foreach($skills as $skill){
				if(!in_array($skill, $currentSkills))
					$DB->execute("insert into martistskill (ma_usruserid, ma_jclid) values (:0, :1)", array($wheres['usr_userid'], $skill));
			}
		}
	}
	return $ret;
