...
$.keyboard = function(el, options){
	...
	base.focusOn = function(){
		...
		if (!base.isVisible() || o.alwaysOpen) {
			...
			base.reveal();
			setTimeout(function(){ if(!base.$preview.is(":focus")) base.$preview.focus(); }, 100);
		}
	};
	...
	base.reveal = function(){
		...
		base.$el.removeClass('ui-keyboard-has-focus');
		base.$el.removeClass('ui-keyboard-input-current');
		...
		if (!o.useDialog) {
			base.$keyboard.removeClass('ui-keyboard');
			if (o.keyboardPlacer) {
				base.$keyboard.appendTo($(o.keyboardPlacer));
			}
			else
				base.$keyboard.appendTo(base.$el.parent());
		...
		if (o.useDialog) {
			...
			if ($.ui.position) {
				...
				var fakeDialog = $();
				fakeDialog.options = false;
				base.overlayDiv = $.ui.dialog.overlay.create(fakeDialog);
				$('body').append(base.overlayDiv)
				
			}
		}
		
		if(!base.$preview.is(":focus")) base.$preview.focus();
	...
	base.startup = function(){
		...
		base.$allKeys
			.bind(o.keyBinding.split(' ').join('.keyboard ') + '.keyboard repeater.keyboard', function(e){
				...
				// base.$preview.trigger(e_keypress);
				var e_keyup = jQuery.Event("keyup");
				base.$preview.trigger(e_keyup);
			...
			.bind('mouseup.keyboard touchend.kb touchmove.kb touchcancel.kb', function(){
				if (!base.$preview.is(":focus")) {
...
console.log(getInputSelection(base.$preview[0]).start,getInputSelection(base.$preview[0]).end);
		...
		// alert(base.$preview.val().substr(0, pos.start - (bksp ? 1 : 0)) + txt + base.$preview.val().substr(pos.end) );
		
		// console.log(pos.start+' '+pos.end);
		// pos.start = 0;
	...
	$.keyboard.defaultOptions = {
		...
		//keyboard placer, use selector string
		keyboardPlacer : false,
		...
		// auto accept content when press Return
		returnAccept : true,
